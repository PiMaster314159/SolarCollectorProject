...
public class SolarCollector : MonoBehaviour
{
    ...
    double density, specificHeatCapacity, thermalConductivity, thermalDiffusivity, fusionTemp; //Values tweaked in Start() based on object material
    ...
    double  EARTH_RADIUS = 6371000, ORBIT_VELOCITY = 7018.528, KARMAN_LINE_HEIGHT = 100000;
    ...
    string tempArrPath = "Assets/TextFiles/tempArray.txt", boolArrPath = "Assets/TextFiles/boolArray.txt", maxTempPath = "Assets/TextFiles/maxTemp.txt", meanTempPath = "Assets/TextFiles/meanTemp.txt";
    ...
    void updateHeat(){
        ...
        //Simulate beam by creating raycasts in shape of cylendar pointed at center of object. If hit point, check if already hit the point. If point has not been hit before, then add to list of hit points.
        Vector3 normal = heatSource.transform.position - transform.position;
        List<GameObject> hitPoints = new List<GameObject>();
        for(float r = (float)beamRadius/10f; r<=beamRadius; r+=(float)beamRadius/10f){
            int count = 0;
            for(float theta = 0; theta<2*Mathf.PI; theta+=0.01f/r){
                ...
                for(int j = 0; j<temps.GetLength(1); j++){
                    for(int k = 0; k<temps.GetLength(2); k++){
                        ...
                        if(NewIsHeated[i,j,k]){
                            double initialTemp = newTemps[i,j,k];
    ...
    IEnumerator writeData(){
        ...
        foreach(double MaxTemp in maxTempList){
            if(System.Math.Round((count * timeStep)/printStep, 10) % 1 == 0){
                WriteString(maxTempPath, "t= " + (printStep*printCount)+": \n"+MaxTemp);
                printCount++;
            }
            count++;
        }
        ...
        foreach(double MeanTemp in meanTempList){
            if(System.Math.Round((count * timeStep)/printStep, 10) % 1 == 0){
                WriteString(meanTempPath, "t= " + (printStep*printCount)+": \n"+MeanTemp);
                printCount++;
            }
            count++;
        }
        count = 0;
        printCount = 0;
        foreach(double[,,] tempArray in tempList){
            if(System.Math.Round((count * timeStep)/printStep, 10) % 1 == 0){
                WriteString(tempArrPath, "t= " + (printStep*printCount)+": \n"+arrToString(tempArray));
                printCount++;
            }
            count++;
        }
        count = 0;
        printCount = 0;
        foreach(bool[,,] heatArray in heatList){
            if(System.Math.Round((count * timeStep)/printStep, 10) % 1 == 0){
                WriteString(boolArrPath, "t= " + (printStep*printCount)+": \n"+arrToString(heatArray));
                printCount++;
            }
            count++;
        }
